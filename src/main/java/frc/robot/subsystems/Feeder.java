...
public class Feeder extends SubsystemBase {
  ...
  public Feeder() {
    ...
    feeder.getConfigurator().apply(motorConfig);
    ...
    m_sysIdRoutine = new SysIdRoutine(
      new SysIdRoutine.Config(
         ...
         (state) -> SignalLogger.writeString("feeder_state", state.toString())
      ),
      new SysIdRoutine.Mechanism(
         (volts) -> feeder.setControl(m_voltReq.withOutput(volts.in(Volts))),
         null,
         this
      )
    );
  ...
  public void runFeederAtVelocity(double velocityMetersPerSec) {
    double radsPerSec = velocityMetersPerSec / Units.inchesToMeters(0.5);
    double rotsPerSec = Units.radiansToRotations(radsPerSec);
    feeder.setControl(voltRequest.withVelocity(rotsPerSec));
  }
  ...
  public void stopFeeder() {feeder.set(0);}
  ...
  public double getVelocity() {
    double rotsPerSec = feeder.getVelocity().getValueAsDouble();
    double radsPerSec = Units.rotationsToRadians(rotsPerSec);
    double metersPerSec = radsPerSec * Units.inchesToMeters(2);
    return metersPerSec;
  }
  public double getAcceleration() {
    double rotsPerSecSquared = feeder.getAcceleration().getValueAsDouble();
  ...
  public double getProfileVelocity() {
    double rotsPerSec = feeder.getClosedLoopReference().getValueAsDouble();
    double radsPerSec = Units.rotationsToRadians(rotsPerSec);
    double metersPerSec = radsPerSec * Units.inchesToMeters(2);
    return metersPerSec;
  }
  public double getProfileAcceleration() {
    double rotsPerSecSquared = feeder.getClosedLoopReferenceSlope().getValueAsDouble();
    double radsPerSecSquared = Units.rotationsToRadians(rotsPerSecSquared);
    double metersPerSecSquared = radsPerSecSquared * Units.inchesToMeters(2);
    return metersPerSecSquared;
  }

  public double getStatorCurrent() {return feeder.getStatorCurrent().getValueAsDouble();}
