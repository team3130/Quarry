...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
    ...
    public final SlewRateLimiter driveLimiter = new SlewRateLimiter(2, -5, 0, 52, 2);
    public final SlewRateLimiter thetaLimiter = new SlewRateLimiter(0, 1000);
        ...
        if(isAngleReal) { //if angle is real, then we were moving 20 ms ago
            if(vector.getNorm() > 0.001){ //if the norm is significant, we continue to move
                ...
                if(cos > 0){ //positive cos means keep moving (turn angle is small)
                    ...
                    driveLimiter.setPositiveRateLimit(driveLimiter.getAccelerationFromPower(600, driveLimiter.lastValue()));
