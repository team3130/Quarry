...
public class Shooter extends SubsystemBase {
  ...
  public double interpolTargetSpeed() {
    ...
    double accelCmd = (rotspersec - slewRateLimiter.lastValue()) / 0.02;
    accelCmd = MathUtil.clamp(accelCmd, -accelerationRotations, accelerationRotations);

    double powerReq = slewRateLimiter.getPowerFromAcceleration(accelCmd, slewRateLimiter.lastValue());
    shooterAccount.setMaxRequest(powerReq);
    double rotAccelMag = slewRateLimiter.getAccelerationFromPower(shooterAccount.getAllowance(), slewRateLimiter.lastValue());
    double rotAccel = Math.copySign(rotAccelMag, accelCmd);

    double newRotVel = slewRateLimiter.calculate(rotAccel);
    return newRotVel;
  }
